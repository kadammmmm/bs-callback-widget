...
## Part 3b: Windows Server Deployment (Self-Hosted)

Use this option if you need to host the backend on your own Windows Server rather than a cloud platform.
...
Copy `wxcc-callback-widget-deploy.zip` to the server and extract it:

```powershell
Expand-Archive -Path wxcc-callback-widget-deploy.zip -DestinationPath C:\Apps
# Creates: C:\Apps\wxcc-callback-widget\
```
...
```powershell
cd C:\Apps\wxcc-callback-widget
.\deploy.ps1
```
...
|--------|---------|-------|
| Backend port | `3000` | Any available TCP port |
| Callback TTL (hours) | `48` | How long callbacks stay in memory |
| Abandon API key | *(blank)* | Leave blank for open access, or enter a secret |
| Admin API key | *(blank)* | Leave blank for open access, or enter a secret |

Then it will automatically:
1. Install backend dependencies via `npm install --omit=dev`
2. Install [PM2](https://pm2.keymetrics.io/) globally (process manager)
3. Start the backend as a PM2 process named `WxCCCallback`
4. Persist it via Windows Task Scheduler so it restarts on reboot
5. Create an inbound Windows Firewall rule for the chosen port
6. Perform a `/health` check and print all network addresses
...
# View live logs
pm2 logs WxCCCallback

# Health check (replace port if you changed it)
...
The script prints all IPv4 addresses at the end. Use the one reachable from WxCC and agent desktops as your `backend-url`. If the server sits behind a reverse proxy or has a DNS name, use that instead.

Example:
```
http://192.168.10.50:3000/api   ← use this in Desktop Layout
http://192.168.10.50:3000/health ← use this to verify
```

### Useful Commands

```powershell
pm2 status                       # Check service state
pm2 logs WxCCCallback            # Live log stream
pm2 restart WxCCCallback         # Restart after config changes
pm2 stop WxCCCallback            # Stop without removing

.\deploy.ps1 -Port 8080          # Install on a different port
.\deploy.ps1 -Uninstall          # Remove service and firewall rule
```

### Environment Variables

To change settings after deployment, edit `backend/ecosystem.config.cjs` (generated by the script), then restart:

```powershell
pm2 restart WxCCCallback
```

Or re-run `.\deploy.ps1` — it will replace the config and restart cleanly.

See `.env.example` in the project root for a description of all supported variables.
